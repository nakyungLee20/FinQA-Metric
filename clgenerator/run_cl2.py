...
def train_model(args, train_pairs, test_pairs, generate_num_ids, 
    ...
    logger.info("Num of Training Data = {}".format(len(contra_pair))) # 34
    ...
    for epoch in range(args.n_epochs):        
        ...
        # json.dump(output_lengths, open("dataset/output_lens.json", "w"), indent=4)

        logger.info("epoch: {}".format(epoch + 1))
        start = time.time()

        for idx in range(len(input_lengths)): #437 (total batch_size divide)
            # alpha warmup
...
if __name__=='__main__':
    ...
    parser.add_argument('--dev_file_1', default='FinQA_mbert_token_val.json', type=str)
    parser.add_argument('--test_file_1', default='FinQA_mbert_token_test.json', type=str)
