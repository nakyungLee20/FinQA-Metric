...
def load_fin_data(filename):  # load the json data to list(dict()) for FinQA dataset
    ...
    for qa in data:
        qa_dict = {}
        qa_dict['original_text'] = qa['qa']['question'] + qa['qa']['explanation']
        qa_dict['equation'] = "x=" + fin_to_math_eq(qa['qa']['program'], qa['table'])
...
def table_to_num(table, row_index, oper):

    arg = 0
    for i in range(len(table[1:])):
        if table[i+1][0] == row_index:
            int_num = []
            for j in range(len(table[i+1][1:])):
                if "(" in table[i + 1][j + 1]:
                  table[i + 1][j + 1]=table[i + 1][j + 1].split("(")[0]
                elif "$" in table[i + 1][j + 1]:
                  table[i + 1][j + 1]=table[i + 1][j + 1].split("$")[1]

                try:
                  num = float(arg_to_num(table[i + 1][j + 1]))
                  int_num.append(num)

                  if oper == " avg ":
                      arg = sum(int_num) / len(int_num)
                  elif oper == " sum ":
                      arg = sum(int_num)
                  elif oper == " min ":
                      arg = min(int_num)
...
def fin_to_math_eq(fin_data, table_data):
    step = fin_data.split("), ")
    step_memory = []

    math_op=["/", "+", "-", "*", "**", ">"]
    table_op=[" avg ", " sum ", " min ", " max "]
    ...
    for idx in range(len(step)):
        ...
        elif operator in table_op:
            arg[0] = table_to_num(table_data, arg[0], operator)
            # print(table_data)
            arg[1] = arg_to_num(arg[1])
            new_eq = "(" + arg[0] + "T" + ")"
...
def prepare_data(pairs_trained, pairs_tested_or_valed, trim_min_count, generate_nums, copy_nums, tree=False, use_bert=False, auto_transformer=False, bert_pretrain_path='bert-base-uncased'):
    input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_or_val_pairs = ()
...
def prepare_contra_train_batch(train_pairs, batch_size, contra_pair_in, subtree_pos_pair_in, args, neg_sample_num, neg_sample_from_pair_file=False):
    ...
    # json.dump(train_pairs, open("../dataset/train_pairs.json", "w"), indent=4)
