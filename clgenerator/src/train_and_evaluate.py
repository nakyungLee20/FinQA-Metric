...
def contra_train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
    ...
    problem_output_ls = []
    total_loss = None
    for nd in range(len(input_batch)):  # nd=0,1,2
        no_expr_loss = False
        ...
        if args.contra_common_tree_pair:
            problem_batch = []
            # print(len(q_list)) # max(output_length)
            for i in range(len(subtree_pos_pair)):  # 16
                pos = subtree_pos_pair[i][nd] if nd < len(subtree_pos_pair[i]) else 0
                # print(subtree_pos_pair[i])  [0,0], len=2
                if pos >= len(q_list):
                    problem_batch.append(q_list[len(q_list)-1][i, :, :])
                else:
                    problem_batch.append(q_list[pos][i, :, :])
            problem_batch = torch.cat(problem_batch, dim=0) # [16,768]
