...
def main():
    ...
    data2 = json.load(open("../dataset/FinQA_mbert_token_train.json"))["pairs"]
    ...
    for i, t1 in enumerate(tqdm.tqdm(tree_ls1)):
        for j, t2 in enumerate(tree_ls2):
            if j <= i:
                continue
            if comp(t1, t2):
                pairs.append([i, j, 0, 0]) # positive pair index add to pairs
    ...
    # print(pairs)
    json.dump(pairs, open("../dataset/pairs/FinQA-FinQA-1.json", "w"), indent=4)
...
def sample(path, sample_num, add_nopair=False, neg_samp=False, contra_sub_tree_pos=False, neg_samp_func=RetTrue):
            ...
            for i in range(5):
                ...
                while(True):
                    if new_pair[0] < len(data1):
                        neg = random.randint(len(data1), len(data)-1)
                        # print(neg)
    ...
    if contra_sub_tree_pos:
        json.dump({"pairs": new_pairs, "pos": pair_pos}, open("../dataset/pairs/FinQA-FinQA-sample1.json", "w"), indent=4)
    else:
        json.dump(new_pairs, open("../dataset/pairs/FinQA-FinQA-sample1.json", "w"), indent=4)


if __name__ == '__main__':
    main() # find all pos pairs
    # total pairs are too much, need to sample
    sample('../dataset/pairs/FinQA-FinQA-1.json', 4, neg_samp=True, contra_sub_tree_pos=True, neg_samp_func=func2)
